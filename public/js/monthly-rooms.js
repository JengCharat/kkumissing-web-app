...
function select_this_room(roomID, roomNumber) {
    // First check if this room has a daily booking
    fetch(`/room-bookings/${roomID}`)
        ...
        .then(data => {
            // Check if there are any active daily bookings
            const hasDailyBooking = data.bookings && data.bookings.some(booking => 
                booking.booking_type === 'daily' && 
                new Date(booking.check_out) >= new Date()
            );

            if (hasDailyBooking) {
                // Show warning for daily tenant room
                alert('ไม่สามารถออกบิลสำหรับห้อง ' + roomNumber + ' ได้ เนื่องจากเป็นห้องพักรายวัน');
                return;
            }

            // Continue with normal room selection process
        ...
        .catch(error => {
            ...
            alert('เกิดข้อผิดพลาดในการตรวจสอบการจองห้อง');
        });
}
